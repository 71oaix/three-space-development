...
本阶段的试行机制对应三种可观察结果：有前置依赖才启用 `intention.frontier`，展示当前可决定的问题和后置问题；事实与取舍混在一起时启用 `intention.fact-decision`，分列已查事实、Agent 建议和用户决定；进入 Specification 前启用 `intention.shared-understanding`，给出可由用户纠正的意图摘要。只有实际做出这些动作，才记录为已使用。
